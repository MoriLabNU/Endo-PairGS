...
from scene.utils import change_ply_format, median_norm
from utils.general_utils import safe_state, build_rotation, knn
import uuid
...
from utils.loss_utils import GradL1Loss, confidence_loss, l_ssim
...
def scene_reconstruction(mp, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        for viewpoint_cam in viewpoint_cams:
            if opt.c2f == True:
                if iteration == 1 or (iteration % opt.c2f_every_step == 0 and iteration < opt.densify_until_iter) :
                    H = viewpoint_cam.image_height
                    W = viewpoint_cam.image_width
                    N = gaussians.get_xyz.shape[0]
                    low_pass = max (H * W / N / (9 * np.pi), 0.3)
                    if opt.c2f_max_lowpass > 0:
                        low_pass = min(low_pass, opt.c2f_max_lowpass)
            ...
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, stage=stage, \
                cam_type=scene.dataset_type, iteration=count, low_pass=low_pass)
        ...
        # print(f"low_pass: {low_pass}, num of gaussian: {N}")
        ...
        # lssim = l_ssim(image_tensor * mask_tensor, gt_image_tensor*mask_tensor) * 0.1
        ...
        if use_depth:
            ...
            # depth_loss = l1_loss(median_norm(depth_tensor), median_norm(gt_depth_tensor), \
        ...
        if use_smooth:
            ...
            # sm_loss = grad_loss(depth_tensor*100, gt_depth_tensor, mask=mask_tensor) * grad_weight
            if not torch.isnan(sm_loss).any():
                loss += sm_loss
        ...
        if use_confidence:
            ...
            if use_depth:
            # #     confidence_loss_dep = confidence_loss(median_norm(gt_depth_tensor), \
            # #         median_norm(depth_tensor), confidences, mask_tensor.unsqueeze(0))*un_dep_weight
                confidence_loss_dep = confidence_loss(gt_depth_tensor/(gt_depth_tensor.max()++1e-6), \
                    depth_tensor/(depth_tensor.max()+1e-6), confidences, mask_tensor.unsqueeze(0))*un_dep_weight
            loss += confidence_loss_img
            if use_depth:
                # print(confidence_loss_dep)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter :
                ...
                if  iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0: # and gaussians.get_xyz.shape[0]< 360000: # 50000
                    # print('Densify')
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    abe_split = True if stage == "coarse" else False
                    gaussians.densify(densify_threshold, opacity_threshold, scene.cameras_extent, size_threshold, 5, 5, scene.model_path, iteration, stage, abe=abe_split)
                    # gaussians.densify(densify_threshold, opacity_threshold, scene.cameras_extent, size_threshold)
                    # print("sigh")
                if  iteration > opt.pruning_from_iter and iteration % opt.pruning_interval == 0: # and gaussians.get_xyz.shape[0]>250000: # 5000
                    # print('Prune')
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
...
def prepare_output_and_logger(expname):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("/suedata1/Free/xkangyu/mycodes/splatt3r_gs/output/", unique_str)
    ...
    logging.basicConfig(filename=os.path.join(args.model_path, "train.log"), level=logging.INFO)
    tb_writer = logging.getLogger()
    # tb_writer.setLevel(logging.INFO)
    # file_h = logging.FileHandler(os.path.join(args.model_path, "train.log"))
    # file_h.setLevel(logging.INFO)
    stream_h = logging.StreamHandler()
    stream_h.setLevel(logging.INFO)
    # tb_writer.addHandler(file_h)
    tb_writer.addHandler(stream_h)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # ssim_test +=ssim(image.unsqueeze(0), gt_image.unsqueeze(0))
                    # lpips_score_test += lpips_score(image.unsqueeze(0), gt_image.unsqueeze(0))
                    l1_test.append(l1_loss(image, gt_image).mean().double().cpu().numpy().tolist())
                    psnr_test.append(psnr(image.unsqueeze(0), gt_image.unsqueeze(0)).mean().double().cpu().numpy().tolist())
                    ssim_test.append(ssim(image.unsqueeze(0), gt_image.unsqueeze(0)).cpu().numpy().tolist())
                    lpips_score_test.append(lpips_score(image.unsqueeze(0), gt_image.unsqueeze(0)).cpu().flatten().numpy().tolist()[0])
                # print(lpips_score_test)
                # psnr_test /= len(config['cameras'])
                # l1_test /= len(config['cameras'])  
                # lpips_score_test /= len(config['cameras'])
                # ssim_test /= len(config['cameras'])  
                # print(psnr_test)
                tb_writer.info("\n[ITER {}] Evaluating {} (lengths: {}): L1 {}, std {}; PSNR {}, std {}; SSIM {}, std {}; lpips_score {}, std {}".format(iteration, config['name'], 
                                                                                                             len(config['cameras']) , stat.mean(l1_test), stat.stdev(l1_test), 
                                                                                                             stat.mean(psnr_test), stat.stdev(psnr_test), stat.mean(ssim_test), 
                                                                                                             stat.stdev(ssim_test), stat.mean(lpips_score_test), stat.stdev(lpips_score_test)))
                # if tb_writer:
