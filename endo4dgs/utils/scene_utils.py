...
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now, dataset_type):
    def render(gaussians, viewpoint, path, scaling, cam_type):
        ...
        confidence_np = confidence.permute(1, 2, 0).cpu().numpy()
        depth_np /= depth_np.max()
        gt_depth /= gt_depth.max()
        confidence_np /= confidence_np.max()
        depth_np = np.repeat(depth_np, 3, axis=2)
        gt_depth = np.repeat(gt_depth, 3, axis=2)
        confidence_np = np.repeat(confidence_np, 3, axis=2)
        # print(confidence_np.shape)
        image_np = np.concatenate((gt_np, image_np, gt_depth, depth_np, confidence_np), axis=1)
        # print(image_np)
