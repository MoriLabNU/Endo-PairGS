...
from pointcept.models.point_transformer_v3 import SerializedPooling, Embedding, SerializedUnpooling
from .point_block import Block
...
class PointTransformerV3Model(nn.Module):
    def __init__(self, in_channels, enable_flash, 
        ...
        if enable_flash:
            enc_patch_size = (60000, 60000, 60000, 60000, 60000)[:len(enc_channels)]
            # enc_patch_size = (1024, 1024, 1024, 1024, 1024)[:len(enc_channels)]
            dec_patch_size=(60000, 60000, 60000, 60000, 60000)[:len(self.dec_channels)]
        else:
    ...
    def forward(self, hidden, rays_pts_emb, scales_emb=None, rotations_emb=None, opacity = None,shs_emb=None, time_emb=None):
        # rays_pts_mean = torch.mean(rays_pts_emb, dim=1, keepdim=True)
        # scales_mean = torch.mean(scales_emb, dim=1, keepdim=True)
        # rotations_mean = torch.mean(rotations_emb, dim=1, keepdim=True)
        # shs_mean = torch.mean(shs_emb.reshape([-1, 48]), dim=1, keepdim=True)

        feat = torch.cat([rays_pts_emb[:, :3], time_emb[:, :1]], dim=1)
        # feat = torch.cat([rays_pts_emb[:, :3], scales_emb[:, :3], rotations_emb[:, :4], opacity[:, :1] , time_emb[:, :1]], dim=1)
        # feat = torch.cat([rays_pts_emb[:, :3], scales_emb, rotations_emb, opacity, shs_emb.reshape([-1, 48]), time_emb], dim=1)
        # model_input = {
        #         'coord': rays_pts_emb[:, :3],
        #         'grid_size': torch.ones([3])*1.0/128,
        #         'offset': torch.tensor([rays_pts_emb.shape[0]]).cumsum(0).to(rays_pts_emb.device),
        #         'feat': feat,
        #     }
        # model_input['grid_coord'] = torch.floor(model_input['coord']*128).int()
        # print(model_input["grid_coord"])
        # x = torch.cat([rays_pts_mean, scales_mean, rotations_mean, opacity, shs_mean, time_emb], dim=1)
        y = self.backbone(feat, hidden)
        return y
    ...
    def __init__(
        ...
        if pdnorm_bn:
            ...
            if turn_off_bn:
                ...
                # bn_layer = partial(nn.LayerNorm, eps=1e-5, elementwise_affine=pdnorm_affine)
        ...
        for s in range(self.num_stages):
            ...
            enc = nn.Sequential()
            if s > 0:
                enc.add_module(module=
                    nn.Sequential(
                        nn.Linear(enc_channels[s - 1], enc_channels[s]),
                        bn_layer(enc_channels[s]),
                        act_layer(),
                    ),
                    name="down",
                    # SerializedPooling(
            ...
            if len(enc) != 0:
                self.enc.add_module(module=enc, name=f"enc{s}")
            ...
            for s in reversed(range(self.num_stages - 1)):
                ...
                dec.add_module(
                    module=nn.Sequential(
                        nn.Linear(dec_channels[s + 1], dec_channels[s]),
                        bn_layer(dec_channels[s]),
                        act_layer(),
                    ),
                    name="up",
                )
                ...
                self.dec.add_module(module=dec, name=f"dec{s}")

    def forward(self, point, hidden):
        # point = Point(data_dict)
        # point.serialization(order=self.order, shuffle_orders=self.shuffle_orders)
        # point.sparsify()
        point_1 = self.embedding(point) + hidden
        point_1 = self.enc(point_1)
        if not self.cls_mode:
            point_1 = self.dec(point_1)
        # else:
