...
class EndoNeRF_Dataset(object):
    ...
    def load_meta(self):
        ...
        if self.is_depth:
            if self.stereomis:
                self.depth_paths = agg_fn('predict_depth')
            else:
                self.depth_paths = agg_fn("predict_depth")
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            # mask
            if len(self.masks_paths) == 0:
                mask = np.ones((self.img_wh[1], self.img_wh[0]), dtype=np.uint8)
            else:
            ...
            if self.is_depth:
                pcd = o3d.io.read_point_cloud(self.image_paths[idx].replace("images", "predict_point_cloud_v4_offset_coord1").replace(".color", "").replace(".png", "_colored.ply"))
                pts_np = np.asarray(pcd.points)
                depth_es = np.expand_dims((pts_np[:, 2]).reshape((self.img_wh[1], self.img_wh[0])), 0).astype(np.float32)
                # depth_es = np.expand_dims(np.load(self.image_paths[idx].replace('images', 'depth_raft').replace('.png', 'depth_raft.png')), 0)
                # depth_es = np.expand_dims(np.array(Image.open(self.image_paths[idx].replace('images', 'depth_raft').replace('.png', 'depth_raft.png'))), 0).astype(np.float32)
                
            else:
                depth_es = np.random.randn(1, self.img_wh[1], self.img_wh[0])
            pc = None
            if idx == 10:
    ...
    def load_pcd(self, quantity=1):
        if quantity == 1:
            pcd = o3d.io.read_point_cloud(self.image_paths[0].replace("images", "predict_point_cloud_v4_offset_coord1").replace(".color", "").replace(".png", "_colored.ply"))
            # print(max(abs(np.asarray(pcd.points, dtype=np.float32)[:,1])))
            # print(max(np.asarray(pcd.points, dtype=np.float32)[:,2]))
            pcd.points = self.get_transform(pcd.points, 0)
            
        elif quantity == 2:
            pcd = o3d.io.read_point_cloud(os.path.join(self.root_dir, "predict_point_cloud_v4thr0.2_offset_pair", "0_30_colored.ply"))
            pcd = pcd.remove_duplicated_points() 
            pcd.points = self.get_transform(pcd.points, 0) # 0
            
        else:
            pcds = []
            for i in range(quantity): # pulling (1, 31)
                pcd = o3d.io.read_point_cloud(self.image_paths[0+i*30].replace("images", "predict_point_cloud_v4_offset_coord1").replace(".color", "").replace(".png", "_colored.ply"))
                pcds.append(pcd)
            pcd = self.merge_point_clouds(pcds)
            pcd.points = self.get_transform(pcd.points, 0)
            
            
        # if len(self.masks_paths) != 0:
        pcd, _ = pcd.remove_statistical_outlier(20, 4)
        
        pts = np.asarray(pcd.points, dtype=np.float32)
        # pts += np.array([[0, 0, 0 * pts[:,2].min()]]) # 0.15 p2_1_1, p3_1 p2_2_2 ; 0.25 p1_1 cutting pulling ; 
        pcd.points = o3d.utility.Vector3dVector(pts)
        # pcd.points = self.get_transform(pcd.points, 10)
        return pcd
    
    def merge_point_clouds(self, pcds: list):
        # 初始化一个空的点云
    ...
    def get_transform(self, pts, idx):
        R, T = self.image_poses[idx]
        R = np.transpose(R)
        w2c = np.concatenate((R, T[...,None]), axis=-1)
        w2c = np.concatenate((w2c, np.array([[0, 0, 0, 1]])), axis=0)
        np_pts = np.asarray(pts, dtype=np.float32)
        pix_ones = np.ones(np_pts.shape[0], np.float32)[:, None]
        # pts = np.asarray(pts, dtype=np.float32)*np.array([[1, -1, -1]])
        pts4 = np.concatenate((np.asarray(pts, dtype=np.float32)*np.array([[1, 1, 1]]), pix_ones), axis=1)
        c2w = np.linalg.inv(w2c)
        pts = (c2w @ pts4.T).T[:, :3]
        # pts += np.array([[0, 0, 0.20 * pts[:,2].min()]])
        print("number of initialized 3D Gaussians:", pts.shape[0])
        return o3d.utility.Vector3dVector(pts)
