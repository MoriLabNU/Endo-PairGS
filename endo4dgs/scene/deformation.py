...
# from .pointtransformer_v3 import PointTransformerV3Model as PTEncoder
...
class Deformation(nn.Module):
    ...
    def query_time(self, rays_pts_emb, scales_emb, rotations_emb, time_feature, time_emb):
        ...
        hidden = self.feature_out[0](hidden)   
        for i in range(1, len(self.feature_out), 2):
            hidden += self.feature_out[i+1](self.feature_out[i](hidden))
    ...
    def forward_dynamic(self, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        # print(rays_pts_emb.shape)
        # print(scales_emb.shape)
        # print(rotations_emb.shape)
        # print(opacity_emb.shape)
        # print(shs_emb.shape)
...
# class PTDeformation(nn.Module):
#     def __init__(self, D=8, W=124, input_ch=27, input_ch_time=9, grid_pe=0, skips=[], args=None):
#         super(PTDeformation, self).__init__()
#         self.D = D
#         self.W = W
#         self.input_ch = input_ch
#         self.input_ch_time = input_ch_time
...
#         self.PT = PTEncoder(in_channels=4, enable_flash=True, enc_dim=64, output_dim=self.W)
#         # print(self.grid.feat_dim)
#         for i in range(self.D-1):
#             self.feature_out.append(nn.ReLU())
#             self.feature_out.append(nn.Linear(self.W,self.W))
...
#     def forward_dynamic(self, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
#         hidden = self.query_time(rays_pts_emb, scales_emb, rotations_emb, time_feature, time_emb)
#         # embeding = self.grid(rays_pts_emb[:, :3], time_emb[:, :1])
#         hidden = self.PT(hidden, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_emb)
#         # hidden = self.PT(embeding, scales_emb, rotations_emb, opacity_emb, shs_emb, time_emb)
#         # hidden = point["feat"]
#         # print(torch.isnan(hidden).any())
#         # print(hidden.shape)
#         # input = torch.cat([rays_pts_emb[:, :3], scales_emb, rotations_emb, opacity_emb, shs_emb.reshape([-1, 48]), time_emb], dim=1)
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        # self.deformation_net = PTDeformation(W=64, D=defor_depth, input_ch=(3)+(3*(posbase_pe))*2, grid_pe=grid_pe, input_ch_time=timenet_output, args=args)
