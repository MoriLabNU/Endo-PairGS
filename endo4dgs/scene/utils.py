...
def change_ply_format(gs_path):
  ...
  o3d.io.write_point_cloud(gs_path.replace("point_cloud.ply", "colored.ply"), pcd)
  
  
def median_norm(input):
    
    b, c, h, w = input.shape
    mid = torch.median(input.reshape([b, c, h*w]), 2, keepdim=True)[0].unsqueeze(1)
    s = torch.sum((input - mid).abs().reshape([b, c, h*w]), 2, keepdim=True).unsqueeze(1)
