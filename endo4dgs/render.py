...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, reconstruct=False):
    render_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "renders")
    depth_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "depth")
    gts_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "gt")
    gtdepth_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "gt_depth")
    masks_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "masks")
    normal_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "normal")
    confidence_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "confidence")
    pcd_path = os.path.join(model_path, name, F_NAME+"{}".format(iteration), "pcd")
    ...
    imageio.mimwrite(os.path.join(model_path, name, F_NAME+"{}".format(iteration), 'ours_video.mp4'), render_array, fps=30, quality=8)
    
    gt_array = torch.stack(gt_list, dim=0).permute(0, 2, 3, 1)
    gt_array = (gt_array*255).clip(0, 255)
    imageio.mimwrite(os.path.join(model_path, name, F_NAME+"{}".format(iteration), 'gt_video.mp4'), gt_array, fps=30, quality=8)
