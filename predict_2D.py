...
class MAST3RGaussians(L.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.decoder = pixelsplat_decoder.DecoderSplattingCUDA_camera0(
...
def predict(config):
    ...
    for data in tqdm(data_loader_test):
        for views in data["context"]:
            ...
            views["camera_pose"] = views["camera_pose"].cuda()
            views["camera_intrinsics"] = views["camera_intrinsics"].cuda()
        for views in data["target"]:
            views["img"] = views["img"].cuda()
            views["maskmap"] = views["maskmap"].cuda()
            views["camera_pose"] = views["camera_pose"].cuda()
            views["camera_intrinsics"] = views["camera_intrinsics"].cuda()
        ...
        with torch.no_grad():
            ...
            color, _ = model.decoder(data, pred1, pred2, (512, 640))
        os.makedirs(os.path.join("./outputs", data["scene"][0], "predict_view1"), exist_ok=True)
        save_image(color[0][0], os.path.join("./outputs", data["scene"][0], "predict_view1", view1["instance"][0]+"render_pt.png"))
        # save_image(color[0][0], os.path.join("./outputs", data["scene"][0], "predict_view1", view1["instance"][0]+"render_v4.png"))
