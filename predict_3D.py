...
from utils.export import save_rgb_ply, save_as_ply
...
def predict(config):
    ...
    if config.use_pretrained:
        ...
        model.encoder.downstream_head1.gaussian_dpt.dpt.load_state_dict(torch.load(os.path.join(config.pretrained_head_path, "endo_retrained_head1_gaussain_dpt.pth")), strict=True)
        model.encoder.downstream_head2.gaussian_dpt.dpt.load_state_dict(torch.load(os.path.join(config.pretrained_head_path, "endo_retrained_head2_gaussain_dpt.pth")), strict=True)
        del ckpt
        model.cuda()
    ...
    for data in tqdm(data_loader_test):
        for views in data["context"]:
            views["img"] = views["img"].cuda()
            views["maskmap"] = views["maskmap"].cuda()
        view1, view2 = data['context']
        with torch.no_grad():
            pred1, pred2 = model.forward(view1, view2)
        os.makedirs(os.path.join(config.save_dir, data["scene"][0], "predict_point_cloud_v4thr0.6_offset_coord1"), exist_ok=True)
        # os.makedirs(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1"), exist_ok=True)
        
        # # save 3D point clouds
        if not os.path.exists(os.path.join(config.save_dir, data["scene"][0], "predict_point_cloud_v4thr0.6_offset_coord1", view1["instance"][0]+"_gaussians.ply")):
            save_rgb_ply(pred1, os.path.join(config.save_dir, data["scene"][0], "predict_point_cloud_v4thr0.6_offset_coord1", view1["instance"][0]+"_gaussians.ply"))
        save_rgb_ply(pred2, os.path.join(config.save_dir, data["scene"][0], "predict_point_cloud_v4thr0.6_offset_coord1", view2["instance"][0]+"_gaussians.ply"))
        
        # save pair point cloud in same coordinate
        # save_as_ply(pred1, pred2, os.path.join(config.save_dir, data["scene"][0], "predict_point_cloud_v4thr0.6_offset_pair", str(view1["idx"][0].numpy())+"_"+str(view2["idx"][0].numpy())+"_gaussians.ply"))
        
        # save depth
        # if not os.path.exists(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1", view1["instance"][0]+"_depth.png")):
        #     depth1 = depth_to_image(pred1["means"][0].cpu().numpy()[:,:,2])
        #     np.save(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1", view1["instance"][0]+"_depth.png").replace(".png", ".npy"), depth1)
        #     depth1_img = Image.fromarray((depth1 * 255).astype(np.uint8))
        #     depth1_img.save(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1", view1["instance"][0]+"_depth.png"))
        
        # depth2 = depth_to_image(pred2["means_in_other_view"][0].cpu().numpy()[:,:,2])
        # np.save(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1", view2["instance"][0]+"_depth.png").replace(".png", ".npy"), depth2)
        # depth2_img = Image.fromarray((depth2 * 255).astype(np.uint8))
        # depth2_img.save(os.path.join(config.save_dir, data["scene"][0], "predict_depth_pixeldiff_offset_coord1", view2["instance"][0]+"_depth.png"))
            
            
def depth_to_image(depth: np.ndarray):
    depth -= depth.min()
    depth /= depth.max()
    return depth #, (depth * 255).astype(np.uint8)
