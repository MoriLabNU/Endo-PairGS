...
class DUST3REndoDataset(torch.utils.data.Dataset):

    def __init__(self, data, coverage, resolution, num_epochs_per_epoch=1, alpha=0.3, beta=0.3):

        super(DUST3REndoDataset, self).__init__()
        self.data = data
        self.coverage = coverage
    ...
    def __getitem__(self, idx):
        ...
        for c_view in context_views:
            ...
            # Transform the input
            view['img'] = self.transform(view['original_img']) # *torch.from_numpy(view["maskmap"])
            view['original_img'] = self.org_transform(view['original_img']) #*torch.from_numpy(view["maskmap"])
        ...
        for t_view in target_views:
            ...
            view = self.data.get_view(sequence, t_view, self.resolution)
            view['original_img'] = self.org_transform(view['original_img'])
            # view["camera_pose"] = np.linalg.pinv(views['context'][0]["camera_pose"]) * view["camera_pose"]
            views['target'].append(view)
...
class DUST3REndoTestDataset(torch.utils.data.Dataset):
