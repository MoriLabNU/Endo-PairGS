...
class GaussianHead(PixelwiseTaskWithDPT):
    ...
    def forward(self, decout, img_shape, mask=None):
        # pass through the heads
        # print(f'deout: {decout[0].shape}')
        pts3d = self.dpt(decout, image_size=(img_shape[0], img_shape[1]))
        # print(pts3d.shape)
        # recover encoder and decoder outputs
        enc_output, dec_output = decout[0], decout[-1]
        ...
        out = torch.cat([pts3d*mask.unsqueeze(dim=1), local_features*mask.unsqueeze(dim=1), gaussian_features*mask.unsqueeze(dim=1)], dim=1)
        if self.postprocess:
