...
class DecoderSplattingCUDA_camera0(torch.nn.Module):
    ...
    def forward(self, batch, pred1, pred2, image_shape):
        ...
        means = pred1["means"].unsqueeze(dim=1)
        covariances = pred1["covariances"].unsqueeze(dim=1)
        harmonics = pred1["sh"].unsqueeze(dim=1)
        opacities = pred1["opacities"].unsqueeze(dim=1)
